...
namespace FreeMote.Plugins
{
    ...
    public class FreeMount : IDisposable
    {
        ...
        /// </summary>
        public static void Init(string path = null, string dllDirPath = null)
        {
            _.InitPlugins(path, dllDirPath);
        }
        ...
        /// </summary>
        /// <param name="path">Base path to find plugins</param>
        public void InitPlugins(string path, string dllDirPath)
        {
            if (string.IsNullOrEmpty(dllDirPath))
            {
                dllDirPath = CurrentPath;
            }
            if (string.IsNullOrEmpty(path))
            {
                path = Path.Combine(dllDirPath, PLUGIN_DIR);
            }
            ...
            AddCatalog(Path.Combine(dllDirPath, PLUGIN_DLL), catalog);
            AddCatalog(Path.Combine(dllDirPath, LIB_DIR, PLUGIN_DLL), catalog); //Allow load in lib folder
            if (Environment.Is64BitProcess)
            {
                AddCatalog(Path.Combine(dllDirPath, PLUGIN_X64_DLL), catalog);
                AddCatalog(Path.Combine(dllDirPath, LIB_DIR, PLUGIN_X64_DLL), catalog); //Allow load in lib folder
            }
